...
#
# --- VERSIONING ---
# 2025-09-22, v1.5 (Helium fork):
#  - Corrected the version detection logic.
#  - The script now stores the installed package version in a `.version` file
#    instead of incorrectly parsing the Chromium engine version from the executable.
#    This fixes the infinite download loop.
#
# 2025-09-22, v1.4 (Helium fork):
...
if [[ -f "$helium_executable_path" && -f "$version_file" ]]; then
	is_installed=true
